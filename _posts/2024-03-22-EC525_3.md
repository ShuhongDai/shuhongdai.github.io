...
toc:
  ...
  - name: (g) A group is said to be abelian if $x \circ y = y \circ x$ for all $x$ and $y$. Show that a group is abelian if and only if
$x \circ y \circ x^{-1} \circ y^{-1} = i$ for all $x$ and $y$.
